...
export class GitModule {
  ...
  branch(): string {
    ...
    const actions = [
      'add', 'update', 'fix', 'remove', 'improve', 'refactor', 'optimize', 'enhance', 'implement',
    ]
    const subjects = [
      'authentication', 'database', 'api', 'ui', 'tests', 'documentation', 'config',
      'validation', 'error-handling', 'logging', 'security', 'performance', 'caching',
    ]
  ...
  commitMessage(): string {
    ...
    const subjects = [
      'user authentication', 'database connection', 'API endpoint', 'UI component', 'test coverage',
      'documentation', 'configuration', 'validation logic', 'error handling', 'logging system',
    ]
  ...
  commitAuthor(options?: { name?: string, email?: string }): string {
    const names = [
      ...
      'Diana Davis', 'Eve Martinez', 'Frank Garcia', 'Grace Wilson', 'Henry Lee',
    ]
