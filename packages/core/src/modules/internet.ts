...
export class InternetModule {
   ...
   */
  domainWord(): string {
    return this.random.arrayElement(this.locale.company.name)
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '')
  }
   ...
   */
  domainSuffix(): string {
    return this.random.arrayElement(this.locale.internet.domainSuffix)
  }
  ...
  slug(wordCount = 3): string {
    ...
    for (let i = 0; i < wordCount; i++) {
      words.push(this.random.arrayElement(this.locale.company.buzzwords))
    }
